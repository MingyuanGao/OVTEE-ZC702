...
/* Global Platform TEE Client API specificaiton implementation. */
		...
		TEEC_Context* context)
{
	...
	if( ret == -1){
		...
		link_init(&context->shared_mem_list);
	}
		...
		TEEC_SharedMemory* sharedMem)
{
	...
	link_init(&sharedMem->head_ref);

	set_link_data(&sharedMem->head_ref, sharedMem);
	add_link(&context->shared_mem_list, &sharedMem->head_ref, TAIL);
	context->shared_mem_cnt++;
	return TEEC_SUCCESS;
		...
		TEEC_SharedMemory* sharedMem)
{
	...
	head= &sharedMem->context->shared_mem_list;
	l = head->next;
	while (l != head) {
		tempSharedMem = l->data;
		if(tempSharedMem) {
	...
	if(found) {
		remove_link(&sharedMem->head_ref);
