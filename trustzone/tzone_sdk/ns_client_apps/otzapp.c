...
/* 	Test application for Trustzone API specification, Crypto, Mutex and 
		...
		unsigned int cipher_action)
{
	...
	if(cipher_action != IGNORE_PARAM) {
		...
		otz_encode_array(&operation_otz, in_data,sizeof(int),OTZ_PARAM_IN);
		if(operation_otz.enc_dec.enc_error_state != OTZ_SUCCESS) {
			perror("encode failed \n");
			goto handle_error_1;
		}
