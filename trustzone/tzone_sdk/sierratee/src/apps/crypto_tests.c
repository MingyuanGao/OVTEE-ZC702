 ...
 * @return 
 */
int test_digest_api(sw_short_int *input_buf,sw_uint input_len,sw_short_int* output_buf,
		...
		sw_uint svc_cmd_id,sw_uint cipher_choice)
{
	...
	sw_short_int* local_buf;

	if((local_buf = sw_malloc((sizeof(sw_short_int))*(*output_len))) == NULL) {
	...
	if((keyObj.dataPtr = sw_malloc((sizeof(sw_short_int))*key_len)) == NULL) {
		TEE_FreeOperation(cipher_test_ops);
		sw_free(local_buf);
