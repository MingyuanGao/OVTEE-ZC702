...
/** @defgroup GP_IntAPI GP Internal API
 ...
 * @return 
 */
TEE_Result TEE_BigIntSetFromOctetString(TEE_BigInt *dest, sw_uint *buffer,
		size_t bufferLen, int sign){
	int iter = 0;
	sw_uint *ptr1, *ptr2;
	...
	ptr2 = (buffer + (bufferLen/sizeof(sw_uint)));
	for(iter = bufferLen; iter > 0; iter--){
		ptr1[iter] = ptr2[iter];
	}
	return TEE_SUCCESS;
	if(!sign){
		*((sw_uint*)dest) = *((sw_uint*)dest) | (1<<((sizeof(sw_uint) * 8) - 1));
	}
}
...
void TEE_BigIntConvertFromS32(TEE_BigInt* dest, int shortVal){
	sw_uint *ptr;
	TEE_MemFill((void *)dest, 0,
			(sizeof(TEE_BigInt) * TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits)));
	ptr = ((sw_uint*)dest + TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits) - 1 );
	*ptr = (sw_uint) shortVal;
}
 ...
 * @return 
 */
sw_uint MaxBitPos(TEE_BigInt* src){
	sw_uint index = 0, val;
 ...
 * @return 
 */
int TEE_BigIntGetBit(TEE_BigInt* src, sw_uint bitIndex){
	int mod, bit;
	sw_uint value;
...
void TEE_BigIntMult(TEE_BigInt* dest, TEE_BigInt* op1, TEE_BigInt* op2){
	TEE_BigInt *temp = (TEE_BigInt*)TEE_Malloc(TEE_BigIntSizeInU32
			(TEE_BigIntNumberOfBits) * 2 * sizeof(TEE_BigInt), 0);
	sw_uint maxBit = MaxBitPos(op2);
	sw_uint x = 0;
	TEE_MemMove((void*)(temp + TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits)),
			(void*)op1, TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt));
	for(x = 0; x <= maxBit; x++){
		if(x)
...
int TEE_BigIntCmp(TEE_BigInt* op1, TEE_BigInt* op2){
	sw_uint *ptr1, *ptr2;
	int len = TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits), i, ret_val = 0;
	ptr1 = (sw_uint*)op1; ptr2 = (sw_uint*)op2;
	if(*ptr1 == *ptr2){
		for(i = 0; i < len; i++){
			if(*(ptr1+i) < *(ptr2+i)){
				ret_val = -1;
	...
	}else if( (*((sw_uint*)op1)) & ((1<<((sizeof(sw_uint) * 8) - 1)) != 0)){
		ret_val = -1;
	}else{
		ret_val = 1;
	}
...
void TEE_BigIntLeftShift(TEE_BigInt* dest, TEE_BigInt* src, size_t bits){
	sw_uint getBit, putBit, value;
	int len = TEE_BigIntSizeInU32(TEE_BigIntNumberOfBits), i, j;
	putBit = 0;
	if(bits >= (sizeof(sw_uint) * 8)){
		TEE_MemMove(src, (src + (bits/(sizeof(sw_uint) * 8))),
				(len - (bits/(sizeof(sw_uint) * 8))) * sizeof(sw_uint));
		TEE_MemFill((src+len-(bits/(sizeof(sw_uint) * 8))), 0, 
				(bits/(sizeof(sw_uint) * 8 ) * sizeof(sw_uint)));
		bits = bits % (sizeof(sw_uint) * 8);
	}
	for(i = len - 1; i >= 0; i--){
		getBit = 0;
		value = *(((sw_uint*)src) + i);
		for(j = 0; j < bits; j++){
			value = *(((sw_uint*)src) + i);
			getBit = (getBit<<1 | (value>>((sizeof(sw_uint) * 8) - j - 1) & 1));
		}
		value = (value<<bits) | putBit;
		*(((sw_uint*)dest) + i) = value;
		putBit = getBit;
 ...
 * @param op2
 */
void TEE_BigIntDivide(TEE_BigInt* dest_q, TEE_BigInt* dest_r,
		TEE_BigInt* op1, TEE_BigInt* op2){
	TEE_BigInt *tmp1 = NULL, *shifted = NULL, *adder = NULL;
	int diffBit;
	tmp1 = (TEE_BigInt*)TEE_Malloc(TEE_BigIntSizeInU32
			(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt), 0);
	shifted = (TEE_BigInt*)TEE_Malloc(TEE_BigIntSizeInU32
			(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt), 0);
	if(!tmp1 || !shifted) {
		sw_seterrno(SW_ENOMEM);
		goto TEE_BigIntDivide_ret;
	}
	TEE_MemMove(tmp1,op1,(TEE_BigIntSizeInU32
			(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt)));
	while(TEE_BigIntCmp(tmp1,op2) >= 0){
		diffBit = MaxBitPos(tmp1) - MaxBitPos(op2);
		TEE_MemMove(shifted,op2,(TEE_BigIntSizeInU32
				(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt)));
		if(diffBit){
			TEE_BigIntLeftShift(shifted, shifted, diffBit);
			adder = (TEE_BigInt*)TEE_Malloc(TEE_BigIntSizeInU32
					(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt), 0);
			if(!adder) {
				sw_seterrno(SW_ENOMEM);
				goto TEE_BigIntDivide_ret;
	...
	TEE_MemMove(dest_r,tmp1,(TEE_BigIntSizeInU32
				(TEE_BigIntNumberOfBits) * sizeof(TEE_BigInt)));

TEE_BigIntDivide_ret:
	if(tmp1)
		TEE_Free((void *)tmp1);
	if(shifted)
		TEE_Free((void *)shifted);
	if(adder)
		TEE_Free((void *)adder);
	return;
}
 ...
 * @param n
 */
void TEE_BigIntMultMod(TEE_BigInt* dest, TEE_BigInt* op1,
 ...
 * @param op2
 */
void TEE_BigIntExtendedGcd(TEE_BigInt* gcd, TEE_BigInt* u, 
 ...
 * @return 
 */
int TEE_BigIntIsProbablePrime(TEE_BigInt* op, sw_uint confidenceLevel){
	//not implemented//
	return TEE_ERROR_NOT_IMPLEMENTED;
}
 ...
 * @param context
 */
void TEE_BigInt2FMM(TEE_BigIntFMM* dest, TEE_BigInt* src,
 ...
 * @param context
 */
void TEE_BigIntFMM2BigInt(TEE_BigInt* dest, TEE_BigIntFMM* src,
		TEE_BigInt* n, TEE_BigIntFMMContext* context){
	//not supported//
}
 ...
 * @param context
 */
void TEE_BigIntFMMCompute(TEE_BigIntFMM* dest, TEE_BigIntFMM* op1,
