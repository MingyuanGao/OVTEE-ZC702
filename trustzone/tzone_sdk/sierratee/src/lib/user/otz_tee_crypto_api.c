 ...
 * @return 
 */
int check_valid_algorithm(uint32_t algorithm)
{
	switch(algorithm) {
 ...
 * @return 
 */
int check_valid_mode(uint32_t mode)
{
	switch(mode) {
 ...
 * @return 
 */
int check_valid_algorithm_for_mode(uint32_t mode,uint32_t algorithm)
{
	switch(mode) {
		case TEE_MODE_ENCRYPT:
		case TEE_MODE_DECRYPT:
 ...
 * @return 
 */
TEEC_Result handleDigestAlloc(TEE_OperationHandle localOperationHandle)
{
 ...
 * @return 
 */
TEEC_Result handleMacAlloc(TEE_OperationHandle localOperationHandle)
{
 ...
 * @return 
 */
TEEC_Result handleCipherAlloc(TEE_OperationHandle localOperationHandle)
{
	u8 load_cipher = 0;
	...
	switch(localOperationHandle->algorithm){
		case TEE_ALG_AES_ECB_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_aes_128_ecb();
			}
		case TEE_ALG_AES_CBC_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_aes_128_cbc();
			}
		case TEE_ALG_AES_CTR:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_aes_128_ctr();
			}
			/*case TEE_ALG_AES_CTS: This case alone merits special treatment*/
		case TEE_ALG_AES_XTS:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_aes_128_xts();
			}
		case TEE_ALG_DES_ECB_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_des_ecb();
			}
		case TEE_ALG_DES_CBC_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_des_cbc();
			}
		case TEE_ALG_DES3_ECB_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_des_ede3();
			}
		case TEE_ALG_DES3_CBC_NOPAD:
			if(load_cipher == 0) {
				load_cipher = 1;
				localOperationHandle->crypto_cipher = (void *)EVP_des_ede3_cbc();
			}
			if((localOperationHandle->crypto_ctxt =
 ...
 * @param operation
 */
void handleDigestFree(TEE_OperationHandle operation)
{
 ...
 * @param operation
 */
void handleMacFree(TEE_OperationHandle operation)
{
	switch(operation->algorithm) {
		case TEE_ALG_HMAC_MD5:
		case TEE_ALG_HMAC_SHA1:
 ...
 * @param operation
 */
void handleCipherFree(TEE_OperationHandle operation)
{
...
void TEE_FreeOperation(TEE_OperationHandle operation)
{
	...
	sw_free(operation);
	sw_printf("Completed Crypto request\n");
	return;
}
 ...
 * @return 
 */
TEEC_Result fillRSAKeys(TEE_OperationHandle operation,
		...
		void* chunk, size_t chunkSize)
{
	...
	switch(operation->algorithm) {
        case TEE_ALG_MD5:
            sw_printf("Request received for MD5:\n");
            break;
        case TEE_ALG_SHA1:
            sw_printf("Request received for SHA1:\n");
            break;
        case TEE_ALG_SHA224:
            sw_printf("Request received for SHA224:\n");
            break;
        case TEE_ALG_SHA256:
            sw_printf("Request received for SHA256:\n");
            break;
        case TEE_ALG_SHA384:
            sw_printf("Request received for SHA384:\n");
            break;
        case TEE_ALG_SHA512:
            sw_printf("Request received for SHA512:\n");
            break;
        default:
            break;
    }
...
void TEE_CipherInit(TEE_OperationHandle operation,void* IV, size_t IVLen)
{
	...
	switch(operation->algorithm) {
        case TEE_ALG_AES_ECB_NOPAD:
            sw_printf("Request received for AES-128-ECB:\n");
            break;
        case TEE_ALG_AES_CBC_NOPAD:
            sw_printf("Request received for AES-128-CBC:\n");
            break;
        case TEE_ALG_AES_CTR:
            sw_printf("Request received for AES-128-CTR:\n");
            break;
        case TEE_ALG_AES_XTS:
            sw_printf("Request received for AES-128-XTS:\n");
            break;
        case TEE_ALG_DES_ECB_NOPAD:
            sw_printf("Request received for DES-128-ECB:\n");
            break;
        case TEE_ALG_DES_CBC_NOPAD:
            sw_printf("Request received for DES-128-CBC:\n");
            break;
        case TEE_ALG_DES3_ECB_NOPAD:
            sw_printf("Request received for 3-DES-128-ECB:\n");
            break;
        case TEE_ALG_DES3_CBC_NOPAD:
            sw_printf("Request received for 3-DES-128-CBC:\n");
            break;
        case TEE_ALG_AES_CTS:
            sw_printf("Request received for AES-128-CTS:\n");
            break;
        default :
            break;
    }
	...
	switch(operation->algorithm) {
		...
		case TEE_ALG_DES3_CBC_NOPAD:
			if(operation->mode == TEE_MODE_ENCRYPT) {
				sw_printf("Performing Cipher Encryption\n");
				if(srcLen != 0) {
				...
				sw_printf("Performing Cipher Decryption\n");
				if(srcLen != 0) {
				...
				if(!EVP_DecryptFinal_ex((EVP_CIPHER_CTX*)(operation->crypto_ctxt),
				destData,(int*)destLen)) {
					return(TEEC_ERROR_GENERIC);
				}
			}
		...
		case TEE_ALG_AES_CTS:
			if(srcLen != 0) {
				if(operation->mode == TEE_MODE_ENCRYPT) {
					sw_printf("Performing TEE_ALG_AES_CTS Encryption\n");
					CRYPTO_cts128_encrypt_block(srcData,destData,srcLen,
							operation->keyValue,operation->IV,
							(block128_f)AES_encrypt);
				} else if(operation->mode == TEE_MODE_DECRYPT) {
					sw_printf("Performing TEE_ALG_AES_CTS Decryption\n");
					CRYPTO_cts128_decrypt_block(srcData,destData,srcLen,
							operation->keyValue,operation->IV,
							(block128_f)AES_encrypt);
				}
...
void TEE_MACInit(TEE_OperationHandle operation,void* IV, size_t IVLen)
{
	...
	switch(operation->algorithm) {
        case TEE_ALG_HMAC_MD5:
            sw_printf("Request received for HMAC-MD5:\n");
            break;
        case TEE_ALG_HMAC_SHA1:
            sw_printf("Request received for HMAC-SHA1:\n");
            break;
        case TEE_ALG_HMAC_SHA224:
            sw_printf("Request received for HMAC-SHA224:\n");
            break;
        case TEE_ALG_HMAC_SHA256:
            sw_printf("Request received for HMAC-SHA256:\n");
            break;
        case TEE_ALG_HMAC_SHA384:
            sw_printf("Request received for HMAC-SHA384:\n");
            break;
        case TEE_ALG_HMAC_SHA512:
            sw_printf("Request received for HMAC-SHA512:\n");
            break;
        default:
            break;
    }
