 ...
 */
typedef struct {
	char mutex_name[MAX_NAME_LEN];
	sw_uint init_param;
	volatile sw_uint lock_param;
	sw_uint owner_id;
	struct link head;
	struct task_pending_queue wait_queue;	
} sw_mutex;
...
* @param lock_val
*/
sw_uint cpu_acquire_mutex(void *lock_val);
...
* @param lock_val
*/
sw_uint cpu_release_mutex(void *lock_val);
...
* @return 
*/
int cpu_acquire_mutex_nowait(void *lock_val);
...
* @param lock_val
*/
sw_uint cpu_acquire_semaphore(void *lock_val);
...
* @param lock_val
*/
sw_uint cpu_release_semaphore(void *lock_val, void* condition);
...
typedef struct {
	...
	sw_uint init_param;
	sw_uint count;
	volatile sw_uint condition;
	volatile sw_uint counter_param;
	struct task_pending_queue wait_queue;
	struct link head;
} sw_semaphore;
