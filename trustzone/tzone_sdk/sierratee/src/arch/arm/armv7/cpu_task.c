...
/* 
 * Architecture specific routines for task implementation
 */
		...
		sw_uint task_sp)
{
	...
	if(task->mode == TASK_KERN_MODE)
		regs->spsr = ARCH_SYS_MODE | IRQ_BIT; /* System mode */
	else
		regs->spsr = ARCH_USR_MODE | IRQ_BIT; /* User mode */
	return SW_OK;
}
...
int  cpu_task_init(void *task)
{
	...
	ret = cpu_task_regs_init(new_task, &new_task->regs, (sw_uint)new_task->task_sp);
	return ret;
}
...
void print_cpu_task_regs(void *task)
{
	...
	for(i=0; i < 13; i++)
		sw_printk("SW: task regs[%x] 0x%x\n", i, pr_task->regs.regs[i]);

	sw_printk("SW: task regs sp 0x%x\n", pr_task->regs.sp);
	sw_printk("SW: task regs lr 0x%x\n", pr_task->regs.lr);
	sw_printk("SW: task regs pc 0x%x\n", pr_task->regs.pc);
}
